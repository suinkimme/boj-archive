...
test.describe('Python 코드 실행기', () => {
  ...
  test('Hello World 실행', async ({ page }) => {
    ...
    await expect(page.locator('#py-run-all-btn')).toBeEnabled({ timeout: 90000 });
  ...
  test('샘플 케이스 실행 — A+B', async ({ page }) => {
    ...
    await page.fill('#py-code', 'a, b = map(int, input().split())\nprint(a + b)');
...
test.describe('언어 전환', () => {
  ...
  test('C++ 전환 시 placeholder 변경', async ({ page }) => {
    await page.selectOption('#runner-lang', 'cpp');
    const placeholder = await page.locator('#py-code').getAttribute('placeholder');
  ...
  test('언어 전환 후 코드 복원', async ({ page }) => {
    ...
    expect(await page.locator('#py-code').inputValue()).toBe('# 저장할 파이썬 코드');
  });

  test('힌트 텍스트가 언어에 맞게 변경', async ({ page }) => {
    expect(await page.locator('#runner-lang-hint').textContent()).toContain('Pyodide');
    await page.selectOption('#runner-lang', 'cpp');
    expect(await page.locator('#runner-lang-hint').textContent()).toContain('JSCPP');
  });
});
