...
test.describe('페이지 로딩', () => {
  test('index.json 로드 후 문제 목록 표시', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('.problem-row').first()).toBeVisible({ timeout: 15000 });
  });
...
test.describe('난이도 티어 필터', () => {
  ...
  test('전체 버튼으로 필터 해제', async ({ page }) => {
    ...
    const filteredNum = parseInt(filteredStat.replace(/[^\d,]/g, '').replace(',', ''));
...
test.describe('페이지네이션', () => {
  ...
  test('다음 페이지 이동', async ({ page }) => {
    const firstTitle = await page.locator('.problem-title').first().textContent();
    await page.click('#next-btn');
    await page.waitForTimeout(200);
    const newFirstTitle = await page.locator('.problem-title').first().textContent();
...
test.describe('문제 모달', () => {
  ...
  test('모달에 문제 제목 표시', async ({ page }) => {
    await page.locator('.problem-row').first().click();
    await expect(page.locator('.modal-title')).not.toBeEmpty({ timeout: 10000 });
  });
