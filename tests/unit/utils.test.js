...
describe('filterProblems', () => {
  ...
  it('tierFilter + tagFilter 조합', () => {
    const result = filterProblems(problems, { tierFilter: '1-5', tagFilter: 'math' });
    expect(result.map(p => p.id)).toEqual([1]);
  });

  it('검색 결과 없으면 빈 배열', () => {
    expect(filterProblems(problems, { query: '존재하지않는문제XYZ' })).toHaveLength(0);
  });
...
describe('getPaginationSlice', () => {
  ...
  it('첫 페이지 슬라이스', () => {
    const { items: page, start, end, totalPages } = getPaginationSlice(items, 1, 50);
