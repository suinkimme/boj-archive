 ...
 *
 * --- MODIFICATION NOTICE ---
 * Source: https://github.com/binji/wasm-clang/blob/HEAD/shared.js
 * Modifications by boj-archive: hostWrite callback signature changed from
 * (str) to (fd, str) so callers can distinguish stdout (fd=1) from stderr
 * (fd=2). All internal API/App log writes pass fd=2; only MemFS.host_write
 * forwards the actual fd from the WASI call. Required so that user-program
 * stdout can be isolated for output comparison without contamination from
 * compiler diagnostics or user stderr.
 */
...
class API {
  ...
  hostLog(message) {
    ...
    this.hostWrite(2, `${yellowArrow}${message}`);
  }
  ...
  async hostLogAsync(message, promise) {
    ...
    const end = +new Date();
    this.hostWrite(2, ' done.');
    if (this.showTiming) {
      const green = '\x1b[92m';
      const normal = '\x1b[0m';
      this.hostWrite(2, ` ${green}(${msToSec(start, end)}s)${normal}\n`);
    }
    this.hostWrite(2, '\n');
    return result;
  }
